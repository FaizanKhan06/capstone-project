...
import PollsComponent from "../CommonComponents/PollsComponent";
...
function CommunityHeadDashboard({ handleOpenSnackbar }) {
  ...
  function getCommunityDetails() {
    fetch(
      "http://localhost:5000/api/communities/communityHead/" +
        retrieveUser().email,
      {
        method: "GET",
        headers: {
  ...
  function getPastCommunityDetails() {
    fetch(
      "http://localhost:5000/api/communities/communityHead/deactive/" +
        retrieveUser().email,
      {
        method: "GET",
        headers: {
  ...
  function getCommunityJoinRequests() {
    if (communityDetails !== null) {
      fetch(
        "http://localhost:5000/api/CommunityMembership/community/" +
  ...
  function rejectUserMembership(membership) {
    ...
    fetch(
      "http://localhost:5000/api/CommunityMembership/" +
        membership.communityMembershipId,
      {
        method: "DELETE",
        headers: {
          Authorization: "Bearer " + retrieveUser().jwtToken,
      ...
      .then((data) => {
        setCommunityMemberships(
          communityMemberships.filter(
            (eachmembership) =>
              eachmembership.communityMembershipId !==
              membership.communityMembershipId
          )
        );
      })
      ...
      {communityDetails !== null && !loading && (
        <Box sx={{ paddingX: "10px", paddingY: "20px" }}>
          ...
          {communityDetails.rule === null ? (
            <>
              <Typography>
                You Have Created A Community but not set the rules
              </Typography>
              <Button
                variant="contained"
                onClick={() => {
                  navigate(getAllUrls(retrieveUser().roleId).createRules, {
                    state: { communityId: communityDetails.communityId },
                  });
                }}
              >
                SET RULE
            ...
            <>
              <CommunityInnerDetailsComponent
              ...
              <Grid container spacing={2} sx={{ marginBottom: 2 }}>
                <Grid item xs={12} sm={6}>
                  ...
                  >
                    ...
                    {
                      <MoneyRequestsComponents
                        handleOpenSnackbar={handleOpenSnackbar}
                        communityDetails={communityDetails}
                        isCommunityHead={true}
              ...
              {!communityDetails.active && communityDetails.deleted && (
                <Grid
                  container
                  spacing={2}
      ...
      {communityDetails === null && !loading && (
        ...
        >
          <Typography variant="h4">
            You have not yet created a Community
          </Typography>
          <Typography>
            You can Create one by clicking on the button below
          </Typography>
          <Button
            variant="contained"
            sx={{ marginY: "10px" }}
            onClick={() =>
              navigate(getAllUrls(retrieveUser().roleId).createCommunity)
            }
          >
            Create a Community
          </Button>
