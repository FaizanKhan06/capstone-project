...
function UserDashboard({ handleOpenSnackbar }) {
  ...
  useEffect(() => {
    setPublicCommunityDetails(
      publicCommunityDetails.filter(
        (publicCommunity) =>
          !userCommunityDetails.some(
            (userCommunity) =>
              userCommunity.communityId === publicCommunity.communityId
          )
      )
    );
  ...
  function getAllJoinedCommunities() {
    fetch(
      "http://localhost:5000/api/CommunityMembership/user/" +
        retrieveUser().email,
      {
        method: "GET",
        headers: {
  ...
  function goToCommunityDetailsWithJoinOption(data) {
    navigate(getAllUrls(retrieveUser().roleId).communityJoin, {
      state: { communityDetails: data },
    });
  }

  function goToCommunityDetails(community) {
    navigate(getAllUrls(retrieveUser().roleId).communityView, {
              ...
              <Box>
                <Accordion>
                  <AccordionSummary
                    ...
                    id="panel1-header"
                  >
                    <Typography component="span">Active Communities</Typography>
                  </AccordionSummary>
                  <AccordionDetails>
                    {userCommunityDetails
                      .filter((each) => each.active === true)
                      .map((userCommunityDetail) => (
                        <Card
                          key={userCommunityDetail.communityId}
                          sx={{
                            padding: 1,
                        ...
                        >
                          <Grid container alignItems="center">
                            <Grid item xs={12} sm={6}>
                              <Button
                                onClick={() =>
                                  goToCommunityDetails(userCommunityDetail)
                                }
                                sx={{
                ...
                <Accordion>
                  ...
                  <AccordionDetails>
                    {userCommunityDetails
                      .filter(
                        (each) =>
                          each.active === false && each.deleted === false
                      )
                      .map((userCommunityDetail) => (
                        <Card
                          key={userCommunityDetail.communityId}
                ...
                <Accordion>
                  <AccordionSummary
                    ...
                    id="panel1-header"
                  >
                    <Typography component="span">Past Communities</Typography>
                  </AccordionSummary>
                  <AccordionDetails>
                    {userCommunityDetails
                      .filter(
                        (each) => each.deleted === true && each.active === false
                      )
                      .map((userCommunityDetail) => (
                        <Card
                          key={userCommunityDetail.communityId}
            ...
            {publicCommunityDetails.length !== 0 && !loading && (
              <Box>
                {publicCommunityDetails.map((publicCommunityDetail) => (
                  <Card
                    key={publicCommunityDetail.communityId}
                    sx={{
                      padding: 1,
                      backgroundColor: "#ECECEC",
                      marginBottom: 1,
                  ...
                  >
                    <Grid container alignItems="center">
                      <Grid item xs={12} sm={6}>
                        <Button
                          onClick={() =>
                            goToCommunityDetailsWithJoinOption(
                      ...
                      >
                        {" "}
                        {/* Left on mobile, right on larger screens */}
                        <Button
                          onClick={() =>
                            goToCommunityDetailsWithJoinOption(
                              publicCommunityDetail
